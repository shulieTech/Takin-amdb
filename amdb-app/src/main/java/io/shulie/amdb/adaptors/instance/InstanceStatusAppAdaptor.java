...
public class InstanceStatusAppAdaptor extends AbstractDefaultAdaptor {

    private static final Logger logger = LoggerFactory.getLogger(InstanceStatusAppAdaptor.class);
        ...
        if (dataContext.getChildPaths() != null) {
            for (Object path : dataContext.getChildPaths()) {
                if (!paths.contains(dataContext.getPath() + "/" + path)) {
                    try {
                        adaptorTemplate.addPath(Connector.ConnectorType.ZOOKEEPER_NODE, dataContext.getPath() + "/" + path, InstanceStatusModel.class, adaptorTemplate.getAdapter(InstanceStatusAdaptor.class));
                    } catch (Exception e) {
